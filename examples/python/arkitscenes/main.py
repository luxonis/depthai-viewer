...
def project_3d_bboxes_to_2d_keypoints(
    ...
    camera_from_world: viewer.TranslationRotationScale3D,
...
def log_camera(
    intri_path: Path,
    frame_id: str,
    poses_from_traj: Dict[str, viewer.TranslationRotationScale3D],
    entity_id: str,
    ...
    viewer.log_transform3d(camera_path, camera_from_world, from_parent=True)
    viewer.log_view_coordinates(camera_path, xyz="RDF")  # X=Right, Y=Down, Z=Forward
    viewer.log_pinhole(f"{entity_id}", child_from_parent=intrinsic, width=w, height=h)


def read_camera_from_world(traj_string: str) -> Tuple[str, viewer.TranslationRotationScale3D]:
    """
    ...
    camera_from_world = viewer.TranslationRotationScale3D(translation, viewer.Quaternion(xyzw=rotation.as_quat()))
